package train2

import (
	"container/heap"
	"container/list"
	"fmt"
	"math"
	"os"
	"strings"
)

type PriorityQueue []*Node

type Node struct {
	station  string
	priority int
	index    int
}

func (pq PriorityQueue) Len() int { return len(pq) }

func (pq PriorityQueue) Less(i, j int) bool {
	return pq[i].priority < pq[j].priority
}

func (pq PriorityQueue) Swap(i, j int) {
	pq[i], pq[j] = pq[j], pq[i]
	pq[i].index = i
	pq[j].index = j
}

func (pq *PriorityQueue) Push(x interface{}) {
	n := len(*pq)
	node := x.(*Node)
	node.index = n
	*pq = append(*pq, node)
}

func (pq *PriorityQueue) Pop() interface{} {
	old := *pq
	n := len(old)
	node := old[n-1]
	old[n-1] = nil
	node.index = -1
	*pq = old[0 : n-1]
	return node
}

// // Station represents a point on the graph with coordinates (for heuristic calculation).
// type Station struct {
// 	X, Y int
// }

// // Graph represents the train network graph.
// type Graph struct {
// 	AdjList  map[string][]string
// 	Stations map[string]Station
// }

// Heuristic function for A* (using Manhattan distance).
func heuristic(from, to Station) int {
	return int(math.Abs(float64(from.X-to.X)) + math.Abs(float64(from.Y-to.Y)))
}

// AStarSearch algorithm finds the shortest path using A* search algorithm.
func AStarSearch(graph *Graph, start, goal string) ([]string, error) {
	goalStation := graph.Stations[goal]

	pq := &PriorityQueue{}
	heap.Init(pq)
	heap.Push(pq, &Node{station: start, priority: 0})

	cameFrom := make(map[string]string)
	costSoFar := make(map[string]int)
	cameFrom[start] = ""
	costSoFar[start] = 0

	for pq.Len() > 0 {
		current := heap.Pop(pq).(*Node).station

		if current == goal {
			path := []string{}
			for current != "" {
				path = append([]string{current}, path...)
				current = cameFrom[current]
			}
			return path, nil
		}

		for _, next := range graph.AdjList[current] {
			newCost := costSoFar[current] + 1
			if cost, ok := costSoFar[next]; !ok || newCost < cost {
				costSoFar[next] = newCost
				priority := newCost + heuristic(graph.Stations[next], goalStation)
				heap.Push(pq, &Node{station: next, priority: priority})
				cameFrom[next] = current
			}
		}
	}

	return nil, fmt.Errorf("no path found from %s to %s", start, goal)
}

// BFS algorithm for comparison or fallback.
func BFS(graph *Graph, start, goal string) ([]string, error) {
	visited := make(map[string]bool)
	queue := list.New()
	queue.PushBack(start)
	visited[start] = true
	cameFrom := make(map[string]string)
	cameFrom[start] = ""

	for queue.Len() > 0 {
		current := queue.Remove(queue.Front()).(string)

		if current == goal {
			path := []string{}
			for current != "" {
				path = append([]string{current}, path...)
				current = cameFrom[current]
			}
			return path, nil
		}

		for _, next := range graph.AdjList[current] {
			if !visited[next] {
				queue.PushBack(next)
				visited[next] = true
				cameFrom[next] = current
			}
		}
	}

	return nil, fmt.Errorf("no path found from %s to %s", start, goal)
}

// HybridSearch combines BFS and A* to find paths based on the number of trains.
func HybridSearch(graph *Graph, start, goal string, numTrains int) ([]string, error) {
	if numTrains > 3 { // Example condition to switch algorithms
		return AStarSearch(graph, start, goal)
	}
	return BFS(graph, start, goal)
}

// MoveTrains simulates the movement of trains from a start to an end station.
// func MoveTrains(graph *Graph, startStation, endStation string, numTrains int) {
// 	trains := make(map[string]string)          // Map train ID to its current station
// 	previousStation := make(map[string]string) // Store previous station of trains
// 	movedAway := make(map[string]bool)         // Track if a train has moved away from the start

// 	// Initialize trains at the starting station
// 	for i := 1; i <= numTrains; i++ {
// 		trainID := fmt.Sprintf("T%d", i)
// 		trains[trainID] = startStation
// 		previousStation[trainID] = "" // Initially, the train has no previous station
// 		movedAway[trainID] = false    // Track that the train has not moved away yet
// 	}

// 	// Station occupation map
// 	occupiedStations := make(map[string]bool)

// 	turns := 0
// 	for {
// 		turns++
// 		turnMovement := []string{}
// 		done := true

// 		// Clear station occupation map, except for the end station
// 		for k := range occupiedStations {
// 			if k != endStation {
// 				delete(occupiedStations, k)
// 			}
// 		}

// 		// Move each train in order
// 		for i := 1; i <= numTrains; i++ {
// 			trainID := fmt.Sprintf("T%d", i)
// 			currentStation := trains[trainID]

// 			if currentStation == endStation {
// 				continue // If the train has reached the destination, it no longer moves
// 			}

// 			// Find path from current station to the destination
// 			path, err := HybridSearch(graph, currentStation, endStation, numTrains)
// 			if err != nil {
// 				fmt.Fprintln(os.Stderr, err)
// 				os.Exit(1)
// 			}

// 			// Check for alternative paths if the next station is occupied
// 			nextStation := path[1]
// 			if (nextStation != endStation && occupiedStations[nextStation]) || nextStation == previousStation[trainID] {
// 				// Attempt to find an alternative path
// 				foundAlternative := false
// 				for _, alternativeStation := range graph.AdjList[currentStation] {
// 					if !occupiedStations[alternativeStation] && alternativeStation != previousStation[trainID] {
// 						// Find a new path from the alternative station to the destination
// 						alternativePath, err := HybridSearch(graph, alternativeStation, endStation, numTrains)
// 						if err == nil && len(alternativePath) > 1 {
// 							nextStation = alternativeStation
// 							//path = alternativePath
// 							foundAlternative = true
// 							break
// 						}
// 					}
// 				}
// 				if !foundAlternative {
// 					// If no alternative path found, the train stays at its current station
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 					continue
// 				}
// 			}

// 			// Move the train to the next station if it's not occupied and not the previous station, except at the end station
// 			if (!occupiedStations[nextStation] || nextStation == endStation) && nextStation != previousStation[trainID] {
// 				previousStation[trainID] = currentStation
// 				trains[trainID] = nextStation
// 				if nextStation != startStation { // Only record if the train has moved away from the starting station
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, nextStation))
// 					movedAway[trainID] = true // Mark the train as having moved away
// 				}
// 				if nextStation != endStation {
// 					occupiedStations[nextStation] = true // Mark the station as occupied unless it's the end station
// 				}
// 				done = false
// 			} else {
// 				// Station is occupied or it's the previous station, the train waits
// 				turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 			}
// 		}

// 		// Print movements of the current turn, but only if there are any movements
// 		if len(turnMovement) > 0 {
// 			// Only include movements for trains that have moved away from the starting station
// 			filteredMovement := []string{}
// 			for _, move := range turnMovement {
// 				trainID := strings.Split(move, "-")[0]
// 				if movedAway[trainID] {
// 					filteredMovement = append(filteredMovement, move)
// 				}
// 			}
// 			if len(filteredMovement) > 0 {
// 				fmt.Println(strings.Join(filteredMovement, " "))
// 			}
// 		}

// 		// End if all trains have reached the destination
// 		if done {
// 			break
// 		}
// 	}
// }

//two-four works
// func MoveTrains(graph *Graph, startStation, endStation string, numTrains int) {
// 	trains := make(map[string]string)          // Map train ID to its current station
// 	previousStation := make(map[string]string) // Store previous station of trains
// 	movedAway := make(map[string]bool)         // Track if a train has moved away from the start

// 	// Initialize trains at the starting station
// 	for i := 1; i <= numTrains; i++ {
// 		trainID := fmt.Sprintf("T%d", i)
// 		trains[trainID] = startStation
// 		previousStation[trainID] = "" // Initially, the train has no previous station
// 		movedAway[trainID] = false    // Track that the train has not moved away yet
// 	}

// 	// Station occupation map
// 	occupiedStations := make(map[string]bool)

// 	// Flag to track whether direct route is occupied
// 	directRouteOccupied := false

// 	turns := 0
// 	for {
// 		turns++
// 		turnMovement := []string{}
// 		done := true

// 		// Clear station occupation map, except for the end station
// 		for k := range occupiedStations {
// 			if k != endStation {
// 				delete(occupiedStations, k)
// 			}
// 		}

// 		// Move each train in order
// 		for i := 1; i <= numTrains; i++ {
// 			trainID := fmt.Sprintf("T%d", i)
// 			currentStation := trains[trainID]

// 			if currentStation == endStation {
// 				continue // If the train has reached the destination, it no longer moves
// 			}

// 			// Find path from current station to the destination
// 			path, err := HybridSearch(graph, currentStation, endStation, numTrains)
// 			if err != nil {
// 				fmt.Fprintln(os.Stderr, err)
// 				os.Exit(1)
// 			}

// 			// Check if the next station is the direct connection
// 			nextStation := path[1]
// 			isDirectRoute := (currentStation == startStation && nextStation == endStation)

// 			if isDirectRoute {
// 				// Only the first train can use the direct route
// 				if directRouteOccupied {
// 					// Skip this train's move and force it to find an alternative route
// 					alternativeFound := false
// 					for _, alternativeStation := range graph.AdjList[currentStation] {
// 						if alternativeStation != endStation && !occupiedStations[alternativeStation] && alternativeStation != previousStation[trainID] {
// 							path, err := HybridSearch(graph, alternativeStation, endStation, numTrains)
// 							if err == nil && len(path) > 1 {
// 								nextStation = alternativeStation
// 								alternativeFound = true
// 								break
// 							}
// 						}
// 					}
// 					if !alternativeFound {
// 						// Train waits at its current station
// 						turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 						continue
// 					}
// 				} else {
// 					directRouteOccupied = true // Mark the direct route as occupied
// 				}
// 			}

// 			// Move the train to the next station if it's not occupied and not the previous station, except at the end station
// 			if (!occupiedStations[nextStation] || nextStation == endStation) && nextStation != previousStation[trainID] {
// 				previousStation[trainID] = currentStation
// 				trains[trainID] = nextStation
// 				if nextStation != startStation { // Only record if the train has moved away from the starting station
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, nextStation))
// 					movedAway[trainID] = true // Mark the train as having moved away
// 				}
// 				if nextStation != endStation {
// 					occupiedStations[nextStation] = true // Mark the station as occupied unless it's the end station
// 				}
// 				done = false
// 			} else {
// 				// Station is occupied or it's the previous station, the train waits
// 				turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 			}
// 		}

// 		// Print movements of the current turn, but only if there are any movements
// 		if len(turnMovement) > 0 {
// 			// Only include movements for trains that have moved away from the starting station
// 			filteredMovement := []string{}
// 			for _, move := range turnMovement {
// 				trainID := strings.Split(move, "-")[0]
// 				if movedAway[trainID] {
// 					filteredMovement = append(filteredMovement, move)
// 				}
// 			}
// 			if len(filteredMovement) > 0 {
// 				fmt.Println(strings.Join(filteredMovement, " "))
// 			}
// 		}

// 		// End if all trains have reached the destination
// 		if done {
// 			break
// 		}
// 	}
// }

// // ALA KOSKE!!!!!!!!!!!!ALMOST WORKS!!!!!!!!!
// func MoveTrains(graph *Graph, startStation, endStation string, numTrains int) {
// 	trains := make(map[string]string)          // Map train ID to its current station
// 	previousStation := make(map[string]string) // Store previous station of trains
// 	movedAway := make(map[string]bool)         // Track if a train has moved away from the start

// 	// Initialize trains at the starting station
// 	for i := 1; i <= numTrains; i++ {
// 		trainID := fmt.Sprintf("T%d", i)
// 		trains[trainID] = startStation
// 		previousStation[trainID] = "" // Initially, the train has no previous station
// 		movedAway[trainID] = false    // Track that the train has not moved away yet
// 	}

// 	// Station occupation map
// 	occupiedStations := make(map[string]bool)
// 	// Connection usage map
// 	usedConnections := make(map[string]bool)

// 	turns := 0
// 	for {
// 		turns++
// 		turnMovement := []string{}
// 		done := true

// 		// Clear station occupation map, except for the end station
// 		for k := range occupiedStations {
// 			if k != endStation {
// 				delete(occupiedStations, k)
// 			}
// 		}

// 		// Clear used connections map for the new turn
// 		usedConnections = make(map[string]bool)

// 		// Move each train in order
// 		for i := 1; i <= numTrains; i++ {
// 			trainID := fmt.Sprintf("T%d", i)
// 			currentStation := trains[trainID]

// 			if currentStation == endStation {
// 				continue // If the train has reached the destination, it no longer moves
// 			}

// 			// Find path from current station to the destination
// 			path, err := HybridSearch(graph, currentStation, endStation, numTrains)
// 			if err != nil {
// 				fmt.Fprintln(os.Stderr, err)
// 				os.Exit(1)
// 			}

// 			// Get the next station from the path
// 			nextStation := path[1]
// 			connection := fmt.Sprintf("%s-%s", currentStation, nextStation)

// 			// Check if the connection is already used
// 			if (nextStation != endStation && occupiedStations[nextStation]) || usedConnections[connection] || nextStation == previousStation[trainID] {
// 				// Attempt to find an alternative path
// 				foundAlternative := false
// 				for _, alternativeStation := range graph.AdjList[currentStation] {
// 					alternativeConnection := fmt.Sprintf("%s-%s", currentStation, alternativeStation)
// 					if !occupiedStations[alternativeStation] && !usedConnections[alternativeConnection] && alternativeStation != previousStation[trainID] {
// 						nextStation = alternativeStation
// 						connection = alternativeConnection
// 						foundAlternative = true
// 						break
// 					}
// 				}
// 				if !foundAlternative {
// 					// If no alternative path found, the train stays at its current station
// 					fmt.Println("1")
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 					continue
// 				}
// 			}

// 			// Move the train to the next station if it's not occupied and not the previous station, except at the end station
// 			if (!occupiedStations[nextStation] || nextStation == endStation) && nextStation != previousStation[trainID] {
// 				previousStation[trainID] = currentStation
// 				trains[trainID] = nextStation
// 				if nextStation != startStation { // Only record if the train has moved away from the starting station
// 					fmt.Println("2")
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, nextStation))
// 					movedAway[trainID] = true // Mark the train as having moved away
// 				}
// 				if nextStation != endStation {
// 					occupiedStations[nextStation] = true // Mark the station as occupied unless it's the end station
// 				}
// 				usedConnections[connection] = true // Mark the connection as used
// 				done = false
// 			} else {
// 				fmt.Println("3")
// 				// Station is occupied or it's the previous station, the train waits
// 				turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 			}
// 		}

// 		// Print movements of the current turn, but only if there are any movements
// 		if len(turnMovement) > 0 {
// 			// Only include movements for trains that have moved away from the starting station
// 			filteredMovement := []string{}
// 			for _, move := range turnMovement {
// 				trainID := strings.Split(move, "-")[0]
// 				if movedAway[trainID] {
// 					filteredMovement = append(filteredMovement, move)
// 				}
// 			}
// 			if len(filteredMovement) > 0 {
// 				fmt.Println(strings.Join(filteredMovement, " "))
// 			}
// 		}

// 		// End if all trains have reached the destination
// 		if done {
// 			break
// 		}
// 	}
// }

// WORKS!!!
func MoveTrains(graph *Graph, startStation, endStation string, numTrains int) {
	trains := make(map[string]string)          // Map train ID to its current station
	previousStation := make(map[string]string) // Store previous station of trains
	movedAway := make(map[string]bool)         // Track if a train has moved away from the start

	// Initialize trains at the starting station
	for i := 1; i <= numTrains; i++ {
		trainID := fmt.Sprintf("T%d", i)
		trains[trainID] = startStation
		previousStation[trainID] = "" // Initially, the train has no previous station
		movedAway[trainID] = false    // Track that the train has not moved away yet
	}

	// Station occupation map
	occupiedStations := make(map[string]bool)
	// Connection usage map
	usedConnections := make(map[string]bool)

	turns := 0
	for {
		turns++
		turnMovement := []string{}
		done := true

		// Clear station occupation map, except for the end station
		for k := range occupiedStations {
			if k != endStation {
				delete(occupiedStations, k)
			}
		}

		// Clear used connections map for the new turn
		usedConnections = make(map[string]bool)

		// Move each train
		for i := 1; i <= numTrains; i++ {
			trainID := fmt.Sprintf("T%d", i)
			currentStation := trains[trainID]

			if currentStation == endStation {
				continue // If the train has reached the destination, it no longer moves
			}

			// Find path from current station to the destination
			path, err := HybridSearch(graph, currentStation, endStation, numTrains)
			if err != nil {
				fmt.Fprintln(os.Stderr, err)
				os.Exit(1)
			}

			// Get the next station from the path
			nextStation := path[1]
			connection := fmt.Sprintf("%s-%s", currentStation, nextStation)

			// Check if the connection is already used
			if (nextStation != endStation && occupiedStations[nextStation]) || usedConnections[connection] || nextStation == previousStation[trainID] {
				// Attempt to find an alternative path or wait
				foundAlternative := false
				for _, alternativeStation := range graph.AdjList[currentStation] {
					alternativeConnection := fmt.Sprintf("%s-%s", currentStation, alternativeStation)
					if !occupiedStations[alternativeStation] && !usedConnections[alternativeConnection] && alternativeStation != previousStation[trainID] {
						nextStation = alternativeStation
						connection = alternativeConnection
						foundAlternative = true
						break
					}
				}
				if !foundAlternative {
					// If no alternative path found, the train stays at its current station
					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
					continue
				}
			}

			// Check if only one train is left that hasn't reached its destination
			remainingTrains := 0
			var remainingTrainID string
			for id := range trains {
				if trains[id] != endStation {
					remainingTrains++
					// remainingTrainID = id
					// remainingTrainID = "T20" + numTrains
					remainingTrainID = fmt.Sprintf("T%d", numTrains)
				}
			}

			// Move the train to the next station if it's not occupied and not the previous station, except at the end station
			if (!occupiedStations[nextStation] || nextStation == endStation) && nextStation != previousStation[trainID] {
				// If only one train is left and a direct path is possible next time, wait and let it go next time directly
				if remainingTrains == 2 && trainID == remainingTrainID && directPathPossible(graph, currentStation, endStation) {
					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
				} else {
					previousStation[trainID] = currentStation
					trains[trainID] = nextStation
					if nextStation != startStation { // Only record if the train has moved away from the starting station
						turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, nextStation))
						movedAway[trainID] = true // Mark the train as having moved away
					}
					if nextStation != endStation {
						occupiedStations[nextStation] = true // Mark the station as occupied unless it's the end station
					}
					usedConnections[connection] = true // Mark the connection as used
					done = false
				}
			} else {
				// Station is occupied or it's the previous station, the train waits
				turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
			}
		}

		// Print movements of the current turn, but only if there are any movements
		if len(turnMovement) > 0 {
			// Only include movements for trains that have moved away from the starting station
			filteredMovement := []string{}
			for _, move := range turnMovement {
				trainID := strings.Split(move, "-")[0]
				if movedAway[trainID] {
					filteredMovement = append(filteredMovement, move)
				}
			}
			if len(filteredMovement) > 0 {
				fmt.Println(strings.Join(filteredMovement, " "))
			}
		}

		// End if all trains have reached the destination
		if done {
			break
		}
	}
}

// func MoveTrains(graph *Graph, startStation, endStation string, numTrains int) {
// 	trains := make(map[string]string)          // Map train ID to its current station
// 	previousStation := make(map[string]string) // Store previous station of trains
// 	movedAway := make(map[string]bool)         // Track if a train has moved away from the start

// 	// Initialize trains at the starting station
// 	for i := 1; i <= numTrains; i++ {
// 		trainID := fmt.Sprintf("T%d", i)
// 		trains[trainID] = startStation
// 		previousStation[trainID] = "" // Initially, the train has no previous station
// 		movedAway[trainID] = false    // Track that the train has not moved away yet
// 	}

// 	// Station occupation map
// 	occupiedStations := make(map[string]bool)

// 	turns := 0
// 	for {
// 		turns++
// 		turnMovement := []string{}
// 		done := true

// 		// Clear station occupation map, except for the end station
// 		for k := range occupiedStations {
// 			if k != endStation {
// 				delete(occupiedStations, k)
// 			}
// 		}

// 		// Clear used connections map for the new turn
// 		usedConnections := make(map[string]bool)

// 		// Move each train
// 		for i := 1; i <= numTrains; i++ {
// 			trainID := fmt.Sprintf("T%d", i)
// 			currentStation := trains[trainID]

// 			if currentStation == endStation {
// 				continue // If the train has reached the destination, it no longer moves
// 			}

// 			// Find path from current station to the destination
// 			path, err := HybridSearch(graph, currentStation, endStation, numTrains)
// 			if err != nil {
// 				fmt.Fprintln(os.Stderr, err)
// 				os.Exit(1)
// 			}

// 			// Get the next station from the path
// 			nextStation := path[1]
// 			connection := fmt.Sprintf("%s-%s", currentStation, nextStation)

// 			// Check if the connection is already used
// 			if (nextStation != endStation && occupiedStations[nextStation]) || usedConnections[connection] || nextStation == previousStation[trainID] {
// 				// Attempt to find an alternative path or wait
// 				foundAlternative := false
// 				for _, alternativeStation := range graph.AdjList[currentStation] {
// 					alternativeConnection := fmt.Sprintf("%s-%s", currentStation, alternativeStation)
// 					if !occupiedStations[alternativeStation] && !usedConnections[alternativeConnection] && alternativeStation != previousStation[trainID] {
// 						nextStation = alternativeStation
// 						connection = alternativeConnection
// 						foundAlternative = true
// 						break
// 					}
// 				}
// 				if !foundAlternative {
// 					// If no alternative path found, the train stays at its current station
// 					fmt.Println("1", strings.Join(turnMovement, " "))
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 					continue
// 				}
// 			}

// 			// Check if only one train is left that hasn't reached its destination
// 			remainingTrains := 0
// 			var remainingTrainID string
// 			for id := range trains {
// 				if trains[id] != endStation {
// 					remainingTrains++
// 					// remainingTrainID = id
// 					// remainingTrainID = "T20" + numTrains
// 					remainingTrainID = fmt.Sprintf("T%d", numTrains)
// 				}
// 			}

// 			// Move the train to the next station if it's not occupied and not the previous station, except at the end station
// 			if (!occupiedStations[nextStation] || nextStation == endStation) && nextStation != previousStation[trainID] {
// 				// If only one train is left and a direct path is possible next time, wait and let it go next time directly
// 				if remainingTrains == 2 && trainID == remainingTrainID && directPathPossible(graph, currentStation, endStation) {
// 					fmt.Println("2", strings.Join(turnMovement, " "))
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 				} else {
// 					previousStation[trainID] = currentStation
// 					trains[trainID] = nextStation
// 					if nextStation != startStation { // Only record if the train has moved away from the starting station
// 						fmt.Println("3", strings.Join(turnMovement, " "))
// 						turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, nextStation))
// 						movedAway[trainID] = true // Mark the train as having moved away
// 					}
// 					if nextStation != endStation {
// 						occupiedStations[nextStation] = true // Mark the station as occupied unless it's the end station
// 					}
// 					usedConnections[connection] = true // Mark the connection as used
// 					done = false
// 				}
// 			} else {
// 				fmt.Println("4", strings.Join(turnMovement, " "))
// 				// Station is occupied or it's the previous station, the train waits
// 				turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 			}
// 		}

// 		// Print movements of the current turn, but only if there are any movements
// 		if len(turnMovement) > 0 {
// 			// Only include movements for trains that have moved away from the starting station
// 			filteredMovement := []string{}
// 			for _, move := range turnMovement {
// 				trainID := strings.Split(move, "-")[0]
// 				if movedAway[trainID] {
// 					filteredMovement = append(filteredMovement, move)
// 				}
// 			}
// 			if len(filteredMovement) > 0 {
// 				fmt.Println(strings.Join(filteredMovement, " "))
// 			}
// 		}

// 		// End if all trains have reached the destination
// 		if done {
// 			break
// 		}
// 	}
// }

//????????/ don't touch yet
// func MoveTrains(graph *Graph, startStation, endStation string, numTrains int) {
// 	trains := make(map[string]string)          // Map train ID to its current station
// 	previousStation := make(map[string]string) // Store previous station of trains
// 	movedAway := make(map[string]bool)         // Track if a train has moved away from the start

// 	// Initialize trains at the starting station
// 	for i := 1; i <= numTrains; i++ {
// 		trainID := fmt.Sprintf("T%d", i)
// 		trains[trainID] = startStation
// 		previousStation[trainID] = "" // Initially, the train has no previous station
// 		movedAway[trainID] = false    // Track that the train has not moved away yet
// 	}

// 	// Station occupation map
// 	occupiedStations := make(map[string]bool)
// 	// Connection usage map
// 	usedConnections := make(map[string]bool)

// 	turns := 0
// 	for {
// 		turns++
// 		turnMovement := []string{}
// 		done := true

// 		// Clear station occupation map, except for the end station
// 		for k := range occupiedStations {
// 			if k != endStation {
// 				delete(occupiedStations, k)
// 			}
// 		}

// 		// Clear used connections map for the new turn
// 		usedConnections = make(map[string]bool)

// 		// Move each train
// 		for i := 1; i <= numTrains; i++ {
// 			trainID := fmt.Sprintf("T%d", i)
// 			currentStation := trains[trainID]

// 			if currentStation == endStation {
// 				continue // If the train has reached the destination, it no longer moves
// 			}

// 			// Find path from current station to the destination
// 			path, err := HybridSearch(graph, currentStation, endStation, numTrains)
// 			if err != nil {
// 				fmt.Fprintln(os.Stderr, err)
// 				os.Exit(1)
// 			}

// 			// Get the next station from the path
// 			nextStation := path[1]
// 			connection := fmt.Sprintf("%s-%s", currentStation, nextStation)

// 			// Check if the connection is already used
// 			if (nextStation != endStation && occupiedStations[nextStation]) || usedConnections[connection] || nextStation == previousStation[trainID] {
// 				// Attempt to find an alternative path or wait
// 				foundAlternative := false
// 				for _, alternativeStation := range graph.AdjList[currentStation] {
// 					alternativeConnection := fmt.Sprintf("%s-%s", currentStation, alternativeStation)
// 					if !occupiedStations[alternativeStation] && !usedConnections[alternativeConnection] && alternativeStation != previousStation[trainID] {
// 						nextStation = alternativeStation
// 						connection = alternativeConnection
// 						foundAlternative = true
// 						break
// 					}
// 				}
// 				if !foundAlternative {
// 					fmt.Println("1")
// 					// If no alternative path found, the train stays at its current station
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 					continue
// 				}
// 			}

// 			// Check if only one train is left that hasn't reached its destination
// 			// remainingTrains := 0
// 			// var remainingTrainID string
// 			// for id := range trains {
// 			// 	if trains[id] != endStation {
// 			// 		remainingTrains++
// 			// 		// remainingTrainID = id
// 			// 		// remainingTrainID = "T20" + numTrains
// 			// 		remainingTrainID = fmt.Sprintf("T%d", numTrains)
// 			// 	}
// 			// }

// 			// Move the train to the next station if it's not occupied, not the previous station, and not the starting station.
// 			if (!occupiedStations[nextStation] || nextStation == endStation) && nextStation != previousStation[trainID] {
// 				// If the train has already moved away from the starting station, it must move.
// 				if movedAway[trainID] {
// 					previousStation[trainID] = currentStation
// 					trains[trainID] = nextStation
// 					fmt.Println("2")
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, nextStation))
// 					if nextStation != endStation {
// 						occupiedStations[nextStation] = true // Mark the station as occupied unless it's the end station
// 					}
// 					usedConnections[connection] = true // Mark the connection as used
// 					done = false
// 				} else {
// 					// Handle the scenario where only one train is left and a direct path is possible
// 					remainingTrains := 0
// 					var remainingTrainID string
// 					for id := range trains {
// 						if trains[id] != endStation {
// 							remainingTrains++
// 							remainingTrainID = fmt.Sprintf("T%d", numTrains)
// 						}
// 					}

// 					// If only one train is left and a direct path is possible next time, wait and let it go next time directly
// 					if remainingTrains == 2 && trainID == remainingTrainID && directPathPossible(graph, currentStation, endStation) {
// 						fmt.Println("3")
// 						turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 					} else {
// 						previousStation[trainID] = currentStation
// 						trains[trainID] = nextStation
// 						if nextStation != startStation { // Only record if the train has moved away from the starting station
// 							fmt.Println("4")
// 							turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, nextStation))
// 							movedAway[trainID] = true // Mark the train as having moved away
// 						}
// 						if nextStation != endStation {
// 							occupiedStations[nextStation] = true // Mark the station as occupied unless it's the end station
// 						}
// 						usedConnections[connection] = true // Mark the connection as used
// 						done = false
// 					}
// 				}
// 			} else if trains[trainID] == startStation {
// 				fmt.Println("5")
// 				// Allow the train to wait only if it is at the starting station
// 				turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 			} else {
// 				// If not at the start station, find an alternative path
// 				foundAlternative := false
// 				for _, alternativeStation := range graph.AdjList[currentStation] {
// 					alternativeConnection := fmt.Sprintf("%s-%s", currentStation, alternativeStation)
// 					if !occupiedStations[alternativeStation] && !usedConnections[alternativeConnection] && alternativeStation != previousStation[trainID] {
// 						nextStation = alternativeStation
// 						connection = alternativeConnection
// 						foundAlternative = true
// 						break
// 					}
// 				}
// 				if foundAlternative {
// 					previousStation[trainID] = currentStation
// 					trains[trainID] = nextStation
// 					fmt.Println("6")
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, nextStation))
// 					if nextStation != endStation {
// 						occupiedStations[nextStation] = true // Mark the station as occupied unless it's the end station
// 					}
// 					usedConnections[connection] = true // Mark the connection as used
// 					done = false
// 				} else {
// 					// If no alternative path is found and the train is not at the start station, force it to move to any available station
// 					for _, alternativeStation := range graph.AdjList[currentStation] {
// 						if alternativeStation != previousStation[trainID] {
// 							previousStation[trainID] = currentStation
// 							trains[trainID] = alternativeStation
// 							fmt.Println("7")
// 							turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, alternativeStation))
// 							if alternativeStation != endStation {
// 								occupiedStations[alternativeStation] = true // Mark the station as occupied unless it's the end station
// 							}
// 							done = false
// 							break
// 						}
// 					}
// 				}
// 			}

// 		}

// 		// Print movements of the current turn, but only if there are any movements
// 		if len(turnMovement) > 0 {
// 			// Only include movements for trains that have moved away from the starting station
// 			filteredMovement := []string{}
// 			for _, move := range turnMovement {
// 				trainID := strings.Split(move, "-")[0]
// 				if movedAway[trainID] {
// 					filteredMovement = append(filteredMovement, move)
// 				}
// 			}
// 			if len(filteredMovement) > 0 {
// 				fmt.Println(strings.Join(filteredMovement, " "))
// 			}
// 		}

// 		// End if all trains have reached the destination
// 		if done {
// 			break
// 		}
// 	}
// }

// func MoveTrains(graph *Graph, startStation, endStation string, numTrains int) {
// 	trains := make(map[string]string)          // Map train ID to its current station
// 	previousStation := make(map[string]string) // Store previous station of trains
// 	movedAway := make(map[string]bool)         // Track if a train has moved away from the start

// 	// Initialize trains at the starting station
// 	for i := 1; i <= numTrains; i++ {
// 		trainID := fmt.Sprintf("T%d", i)
// 		trains[trainID] = startStation
// 		previousStation[trainID] = "" // Initially, the train has no previous station
// 		movedAway[trainID] = false    // Track that the train has not moved away yet
// 	}

// 	// Station occupation map
// 	occupiedStations := make(map[string]bool)

// 	//CHANGE 1
// 	// Flag to track whether direct route is occupied
// 	//directRouteOccupied := false

// 	turns := 0
// 	for {
// 		turns++
// 		turnMovement := []string{}
// 		done := true

// 		// Clear station occupation map, except for the end station
// 		for k := range occupiedStations {
// 			if k != endStation {
// 				delete(occupiedStations, k)
// 			}
// 		}

// 		// Clear used connections map for the new turn
// 		usedConnections := make(map[string]bool)

// 		// Move each train
// 		for i := 1; i <= numTrains; i++ {
// 			trainID := fmt.Sprintf("T%d", i)
// 			currentStation := trains[trainID]

// 			if currentStation == endStation {
// 				continue // If the train has reached the destination, it no longer moves
// 			}

// 			// Find path from current station to the destination
// 			path, err := HybridSearch(graph, currentStation, endStation, numTrains)
// 			if err != nil {
// 				fmt.Fprintln(os.Stderr, err)
// 				os.Exit(1)
// 			}

// 			// Get the next station from the path
// 			nextStation := path[1]
// 			connection := fmt.Sprintf("%s-%s", currentStation, nextStation)

// 			//CHANGE 2
// 			// isDirectRoute := (currentStation == startStation && nextStation == endStation)
// 			// if isDirectRoute {
// 			// 	// Only the first train can use the direct route!!!!!! CHANGE IT!!!!!!
// 			// 	if directRouteOccupied {
// 			// 		// Skip this train's move and force it to find an alternative route
// 			// 		alternativeFound := false
// 			// 		for _, alternativeStation := range graph.AdjList[currentStation] {
// 			// 			if alternativeStation != endStation && !occupiedStations[alternativeStation] && alternativeStation != previousStation[trainID] {
// 			// 				path, err := HybridSearch(graph, alternativeStation, endStation, numTrains)
// 			// 				if err == nil && len(path) > 1 {
// 			// 					nextStation = alternativeStation
// 			// 					alternativeFound = true
// 			// 					break
// 			// 				}
// 			// 			}
// 			// 		}
// 			// 		if !alternativeFound {
// 			// 			// Train waits at its current station
// 			// 			turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 			// 			continue
// 			// 		}
// 			// 	} else {
// 			// 		directRouteOccupied = true // Mark the direct route as occupied
// 			// 	}
// 			// }
// 			//change2 end

// 			// Check if the connection is already used
// 			if (nextStation != endStation && occupiedStations[nextStation]) || usedConnections[connection] || nextStation == previousStation[trainID] {
// 				// Attempt to find an alternative path or wait
// 				foundAlternative := false
// 				for _, alternativeStation := range graph.AdjList[currentStation] {
// 					alternativeConnection := fmt.Sprintf("%s-%s", currentStation, alternativeStation)
// 					if !occupiedStations[alternativeStation] && !usedConnections[alternativeConnection] && alternativeStation != previousStation[trainID] {
// 						nextStation = alternativeStation
// 						connection = alternativeConnection
// 						foundAlternative = true
// 						break
// 					}
// 				}
// 				if !foundAlternative {
// 					// If no alternative path found, the train stays at its current station
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 					continue
// 				}
// 			}

// 			// Check if only one train is left that hasn't reached its destination
// 			// remainingTrains := 0
// 			// var remainingTrainID string
// 			// for id := range trains {
// 			// 	if trains[id] != endStation {
// 			// 		remainingTrains++
// 			// 		remainingTrainID = fmt.Sprintf("T%d", numTrains)
// 			// 	}
// 			// }

// 			// Check if the total number of trains is even
// 			isEven := numTrains%2 == 0
// 			//isTwoTrains := numTrains == 2

// 			// Move the train to the next station if it's not occupied and not the previous station, except at the end station
// 			if (!occupiedStations[nextStation] || nextStation == endStation) && nextStation != previousStation[trainID] {
// 				// If only one train is left and a direct path is possible next time, wait and let it go next time directly
// 				// if remainingTrains == 2 && trainID == remainingTrainID && directPathPossible(graph, currentStation, endStation) {
// 				// 	turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 				// }

// 				if isEven && i == numTrains && directPathPossible(graph, currentStation, endStation) {
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 				} else {
// 					previousStation[trainID] = currentStation
// 					trains[trainID] = nextStation
// 					if nextStation != startStation { // Only record if the train has moved away from the starting station
// 						turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, nextStation))
// 						movedAway[trainID] = true // Mark the train as having moved away
// 					}
// 					if nextStation != endStation {
// 						occupiedStations[nextStation] = true // Mark the station as occupied unless it's the end station
// 					}
// 					usedConnections[connection] = true // Mark the connection as used
// 					done = false
// 				}
// 			} else {
// 				// Station is occupied or it's the previous station, the train waits
// 				turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 			}
// 		}

// 		// Print movements of the current turn, but only if there are any movements
// 		if len(turnMovement) > 0 {
// 			// Only include movements for trains that have moved away from the starting station
// 			filteredMovement := []string{}
// 			for _, move := range turnMovement {
// 				trainID := strings.Split(move, "-")[0]
// 				if movedAway[trainID] {
// 					filteredMovement = append(filteredMovement, move)
// 				}
// 			}
// 			if len(filteredMovement) > 0 {
// 				fmt.Println(strings.Join(filteredMovement, " "))
// 			}
// 		}

// 		// End if all trains have reached the destination
// 		if done {
// 			break
// 		}
// 	}
// }

func directPathPossible(graph *Graph, currentStation, endStation string) bool {
	// Get the list of directly connected stations from the current station using the graph's adjacency list
	connectedStations := graph.AdjList[currentStation]

	// Check if the end station is in the list of directly connected stations
	for _, station := range connectedStations {
		if station == endStation {
			return true
		}
	}

	// If not found, return false
	return false
}

// // // ShortestPathDistance calculates the shortest distance from the start station to the destination station
// // func ShortestPathDistance(graph *Graph, startStation, endStation string) (int, error) {
// // 	// BFS queue
// // 	queue := []string{startStation}
// // 	// Distance map to keep track of distances from the start station
// // 	distances := make(map[string]int)
// // 	distances[startStation] = 0

// // 	// BFS traversal
// // 	for len(queue) > 0 {
// // 		currentStation := queue[0]
// // 		queue = queue[1:]

// // 		// If we reached the destination, return the distance
// // 		if currentStation == endStation {
// // 			return distances[currentStation], nil
// // 		}

// // 		// Traverse all neighbors of the current station
// // 		for _, neighbor := range graph.AdjList[currentStation] {
// // 			if _, visited := distances[neighbor]; !visited {
// // 				// If the neighbor hasn't been visited, add it to the queue and update its distance
// // 				queue = append(queue, neighbor)
// // 				distances[neighbor] = distances[currentStation] + 1
// // 			}
// // 		}
// // 	}

// // 	// If the destination is unreachable, return an error
// // 	return -1, fmt.Errorf("no path from %s to %s", startStation, endStation)
// // }

// func CalculateMinTurns(graph *Graph, startStation, endStation string, numTrains int) int {
// 	// Максимально допустимое количество задержек на начальной станции.
// 	maxDelay := numTrains

// 	// Минимальное количество кругов для завершения.
// 	minTurns := math.MaxInt32

// 	for delay := 0; delay <= maxDelay; delay++ {
// 		turns := SimulateWithDelay(graph, startStation, endStation, numTrains, delay)
// 		if turns < minTurns {
// 			minTurns = turns
// 		}
// 	}

// 	return minTurns
// }

// // SimulateWithDelay запускает симуляцию с задержкой поездов на начальной станции.
// func SimulateWithDelay(graph *Graph, startStation, endStation string, numTrains, delay int) int {
// 	trains := make(map[string]string)
// 	previousStation := make(map[string]string)
// 	movedAway := make(map[string]bool)
// 	occupiedStations := make(map[string]bool)

// 	// Инициализация поездов на начальной станции
// 	for i := 1; i <= numTrains; i++ {
// 		trainID := fmt.Sprintf("T%d", i)
// 		trains[trainID] = startStation
// 		previousStation[trainID] = ""
// 		movedAway[trainID] = false
// 	}

// 	turns := 0
// 	for {
// 		turns++
// 		turnMovement := []string{}
// 		done := true
// 		directRouteUsedThisTurn := false

// 		// Очистка карты занятых станций, за исключением конечной станции
// 		for k := range occupiedStations {
// 			if k != endStation {
// 				delete(occupiedStations, k)
// 			}
// 		}

// 		for i := 1; i <= numTrains; i++ {
// 			trainID := fmt.Sprintf("T%d", i)
// 			currentStation := trains[trainID]

// 			if currentStation == endStation {
// 				continue
// 			}

// 			// Если текущий поезд должен задержаться на начальной станции
// 			if turns <= delay {
// 				turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, startStation))
// 				done = false
// 				continue
// 			}

// 			// Поиск пути от текущей станции до конечной
// 			path, err := HybridSearch(graph, currentStation, endStation, numTrains)
// 			if err != nil {
// 				fmt.Fprintln(os.Stderr, err)
// 				os.Exit(1)
// 			}

// 			nextStation := path[1]
// 			isDirectRoute := (currentStation == startStation && nextStation == endStation)

// 			if isDirectRoute {
// 				if directRouteUsedThisTurn {
// 					alternativeFound := false
// 					for _, alternativeStation := range graph.AdjList[currentStation] {
// 						if alternativeStation != endStation && !occupiedStations[alternativeStation] && alternativeStation != previousStation[trainID] {
// 							path, err := HybridSearch(graph, alternativeStation, endStation, numTrains)
// 							if err == nil && len(path) > 1 {
// 								nextStation = alternativeStation
// 								alternativeFound = true
// 								break
// 							}
// 						}
// 					}
// 					if !alternativeFound {
// 						turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 						continue
// 					}
// 				} else {
// 					directRouteUsedThisTurn = true
// 				}
// 			}

// 			if (!occupiedStations[nextStation] || nextStation == endStation) && nextStation != previousStation[trainID] {
// 				previousStation[trainID] = currentStation
// 				trains[trainID] = nextStation
// 				if nextStation != startStation {
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, nextStation))
// 					movedAway[trainID] = true
// 				}
// 				if nextStation != endStation {
// 					occupiedStations[nextStation] = true
// 				}
// 				done = false
// 			} else {
// 				turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 			}
// 		}

// 		if done {
// 			break
// 		}
// 	}

// 	return turns
// }

// func MoveTrains(graph *Graph, startStation, endStation string, numTrains int) {
// 	// Calculate the minimum turns required.
// 	minTurns := CalculateMinTurns(graph, startStation, endStation, numTrains)
// 	fmt.Printf("Minimum turns needed: %d\n", minTurns)
// }

// func MoveTrains(graph *Graph, startStation, endStation string, numTrains int) {
// 	trains := make(map[string]string)          // Карта текущих станций для каждого поезда
// 	previousStation := make(map[string]string) // Карта предыдущих станций для каждого поезда
// 	movedAway := make(map[string]bool)         // Отслеживание поездов, покинувших начальную станцию
// 	remainingTrains := numTrains

// 	// Инициализация поездов на начальной станции
// 	for i := 1; i <= numTrains; i++ {
// 		trainID := fmt.Sprintf("T%d", i)
// 		trains[trainID] = startStation
// 		previousStation[trainID] = "" // Изначально у поезда нет предыдущей станции
// 		movedAway[trainID] = false    // Пометка, что поезд еще не покинул начальную станцию
// 	}

// 	turns := 0
// 	for {
// 		turns++
// 		turnMovement := []string{}
// 		done := true

// 		// Флаг для отслеживания, был ли использован прямой маршрут в этом обороте
// 		directRouteUsedThisTurn := false

// 		// Определяем сколько поездов могут двигаться в текущем обороте, чтобы минимизировать количество кругов
// 		maxDirectRoutes := remainingTrains / 2
// 		directRoutesUsed := 0

// 		// Перемещение каждого поезда по порядку
// 		for i := 1; i <= numTrains; i++ {
// 			trainID := fmt.Sprintf("T%d", i)
// 			currentStation := trains[trainID]

// 			if currentStation == endStation {
// 				continue // Если поезд достиг конечной станции, он больше не движется
// 			}

// 			// Поиск пути от текущей станции до конечной
// 			path, err := HybridSearch(graph, currentStation, endStation, numTrains)
// 			if err != nil {
// 				fmt.Fprintln(os.Stderr, err)
// 				os.Exit(1)
// 			}

// 			// Определение следующей станции
// 			nextStation := path[1]
// 			isDirectRoute := (currentStation == startStation && nextStation == endStation)

// 			if isDirectRoute {
// 				// Только ограниченное количество поездов может использовать прямой маршрут за один оборот
// 				if directRoutesUsed < maxDirectRoutes {
// 					directRoutesUsed++
// 					directRouteUsedThisTurn = true // Отметить, что прямой маршрут использован
// 				} else {
// 					// Пропустить ход этого поезда и заставить его искать альтернативный маршрут
// 					alternativeFound := false
// 					for _, alternativeStation := range graph.AdjList[currentStation] {
// 						if alternativeStation != endStation && !contains(trains, alternativeStation) && alternativeStation != previousStation[trainID] {
// 							path, err := HybridSearch(graph, alternativeStation, endStation, numTrains)
// 							if err == nil && len(path) > 1 {
// 								nextStation = alternativeStation
// 								alternativeFound = true
// 								break
// 							}
// 						}
// 					}
// 					if !alternativeFound {
// 						// Если альтернативный путь не найден и поезд остаётся на начальной станции
// 						turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 						continue
// 					}
// 				}
// 			}

// 			// Перемещение поезда на следующую станцию, если она не занята и не является предыдущей
// 			if nextStation != previousStation[trainID] {
// 				previousStation[trainID] = currentStation
// 				trains[trainID] = nextStation
// 				if nextStation != startStation { // Запись движения, если поезд покинул начальную станцию
// 					turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, nextStation))
// 					movedAway[trainID] = true // Пометка, что поезд покинул начальную станцию
// 				}
// 				done = false
// 			} else {
// 				// Станция занята или это предыдущая станция, поезд ждет
// 				turnMovement = append(turnMovement, fmt.Sprintf("%s-%s", trainID, currentStation))
// 			}
// 		}

// 		// Вывод движений за текущий оборот, если они были
// 		if len(turnMovement) > 0 {
// 			// Включить только движения поездов, которые покинули начальную станцию
// 			filteredMovement := []string{}
// 			for _, move := range turnMovement {
// 				trainID := strings.Split(move, "-")[0]
// 				if movedAway[trainID] {
// 					filteredMovement = append(filteredMovement, move)
// 				}
// 			}
// 			if len(filteredMovement) > 0 {
// 				fmt.Println(strings.Join(filteredMovement, " "))
// 			}
// 		}

// 		// Завершить, если все поезда достигли конечной станции
// 		if done {
// 			break
// 		}

// 		remainingTrains = 0
// 		for i := 1; i <= numTrains; i++ {
// 			trainID := fmt.Sprintf("T%d", i)
// 			if trains[trainID] != endStation {
// 				remainingTrains++
// 			}
// 		}
// 	}
// }

// // Проверка наличия станции в карте поездов
// func contains(trains map[string]string, station string) bool {
// 	for _, v := range trains {
// 		if v == station {
// 			return true
// 		}
// 	}
// 	return false
// }

func ValidateExtraArgs(args []string) {
	for _, arg := range args {
		if arg == "extra" || arg == "bonus" {
			fmt.Println("Handling extra argument:", arg)
		} else {
			fmt.Fprintf(os.Stderr, "Error: Invalid extra argument: %s\n", arg)
			os.Exit(1)
		}
	}
}
